...
check_password(User, AuthzId, Server, Password) ->
    ?DEBUG("====111==: \"~s, ~s, ~s\"", [AuthzId, User, Password]),
            ...
            case is_scrammed() of
                ...
                false ->
                    try sql_queries:get_password(LServer, LUser) of
                        {selected, [{Password}]} ->
                            ?DEBUG("====112==: \"~s\"", [Password]),
                            Password /= <<"">>;
                        {selected, [{_Password2}]} ->
                            ?DEBUG("====112cccc==: \"~s\"", [_Password2]),
...
check_password(User, AuthzId, Server, Password, Digest,
	       DigestGen) ->
    ?DEBUG("===222==: \"~s, ~s, ~s\"", [AuthzId, User, Password]),
    if AuthzId /= <<>> andalso AuthzId /= User ->
        false;
    true ->
        LServer = jid:nameprep(Server),
